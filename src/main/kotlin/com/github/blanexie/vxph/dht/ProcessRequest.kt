...
class ProcessRequest(val bucket: Bucket) {
    fun replyPack(dictionary: Map<String, Any>): ByteArray? {
        ...
        when (q) {
            "ping" -> return processPing(dictionary)
            "find_node" -> return processFindNode(dictionary)
            "get_peers" -> return processGetPeers(dictionary)
    ...
    private fun processGetPeers(dict: Map<String, Any>): ByteArray {
        val a = dict.readMap("a")
        val id = a.readByteArray("id")
        bucket.addNode(id)
